...
public class PanelCartasJugadorOmaha extends JPanel implements Observer{
    private JugadorOmaha jugador;
    ...
    public PanelCartasJugadorOmaha(JugadorOmaha jugador,Controller ctrl) {
        this.ctrl = ctrl;
    ...
    private void iniGUI() {
        ...
        try {
            BufferedImage myPicture = ImageIO.read(new File(jugador.getCarta4().getCartaImagen()));
            image = myPicture;
            img = new ImageIcon(image.getScaledInstance(60, 80, Image.SCALE_SMOOTH));
            carta2 = new JLabel(img);
            carta2.setBounds(55, 40, 60, 80);
            add(carta2);
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
        try {
            BufferedImage myPicture = ImageIO.read(new File(jugador.getCarta3().getCartaImagen()));
            image = myPicture;
            img = new ImageIcon(image.getScaledInstance(60, 80, Image.SCALE_SMOOTH));
            carta2 = new JLabel(img);
            carta2.setBounds(55, 0, 60, 80);
            add(carta2);
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    ...
    private void setBoundsJugador() {
        switch (jugador.getNumJugador()) {
            case 1:
                setBounds(60, 100, 115, 140);
                break;
            case 2:
                setBounds(60, 400, 115, 140);
                break;
            case 3:
                setBounds(350, 400, 115, 140);
                break;
            case 4:                              
                setBounds(600, 400, 115, 140);
                break;
            case 5:
                setBounds(600, 100, 115, 140);
                break;
            case 6:
                setBounds(350, 50, 115, 140);
                break;
        }
    }
